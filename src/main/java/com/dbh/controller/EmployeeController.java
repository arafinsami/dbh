...
public class EmployeeController {
    ...
    public ResponseEntity<JSONObject> save(@Valid @RequestBody EmployeeRequest request, BindingResult bindingResult) throws JsonProcessingException {
    ...
    @PutMapping
    public ResponseEntity<JSONObject> update(@Valid @RequestBody EmployeeRequest dto, BindingResult bindingResult) throws JsonProcessingException {
        if (bindingResult.hasErrors()) {
            return badRequest().body(error(fieldError(bindingResult)).getJson());
        }
        Employee employee = employeeService.findById(dto.getId()).orElseThrow();
        employeeMapper.update(employee, dto);
        Employee employeeResponse = employeeService.update(employee);
        EmployeeResponse response = employeeMapper.from(employeeResponse);
    ...
    public ResponseEntity<JSONObject> findById(@PathVariable Long id) throws JsonProcessingException {
